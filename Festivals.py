...
def create_Fesival_pdf(filename, ps, pagesize, title="Festivals"):
    ...
    try:
        ...
        for i in range(len(festivalevents)):
            ...
            c.drawString(festival_x + 125, festival_y + 115, str(festivalevents[i].endday))
            c.drawString(festival_x + 132, festival_y + 115, monthnames[festivalevents[i].endmonth-1])
            c.setFillColor(black)
            c.setFont(festivalfont, 25)
